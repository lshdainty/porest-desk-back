...
public interface MemoTagService {
     ...
     *
     * <p>엔티티가 아니라 {@link MemoTagServiceDto.TagRef} 를 돌려준다 — 확보는 <b>새 트랜잭션</b>에서
     * 돌므로 거기서 만든 엔티티는 부르는 쪽 영속성 컨텍스트에 속하지 않는다(할 일 태그와 같은 이유).
     * 그렇다고 {@code rowId} 만 주면 부르는 쪽이 이름을 읽으려고 그 행을 다시 조회하게 되는데,
     * 방금 커밋된 행은 부르는 트랜잭션의 스냅샷에 없어 <b>안 보인다</b>(QA #102). 그래서
     * <b>보이는 자리에서</b> 읽은 이름·색을 함께 실어 보낸다.
     */
    MemoTagServiceDto.TagRef findOrCreateByName(Long userRowId, String rawTagName);
}
