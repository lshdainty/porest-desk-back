...
public class TodoTagServiceDto {
     ...
     * 메모 태그({@code MemoTagServiceDto.TagRef})와 같은 이유·같은 모양이다.
     *
     * <p>확보는 새 트랜잭션에서 돌고, 부르는 트랜잭션은 그 커밋 전에 이미 스냅샷을 잡아 둔
     * 상태라 그 행을 <b>다시 읽을 수 없다</b>(MariaDB REPEATABLE READ · #310 실측).
     * 그래서 부르는 쪽은 FK 를 프록시({@code getReference})로 잇는데, 프록시에서 이름·색을
     * 읽으면 거기서 지연 로딩 SELECT 가 나가 같은 이유로 터진다({@code EntityNotFoundException}).
     * 확보가 <b>보이는 자리에서</b> 읽어 둔 값을 여기 실어 보내는 이유다.
     */
    public record TagRef(
        ...
        public static TagRef from(TodoTag tag) {
