...
public class NamuProperties {
     ...
     * 환율 캐시 유지 시간 (초).
     *
     * <p><b>왜 시세(20초)보다 훨씬 긴가</b> — 여기서 받아 오는 값이 <b>하루짜리 고정값</b>이기
     * 때문이다. 1순위는 해외 잔고의 <b>당일</b>매매기준환율({@code tdt_sby_bse_xcg_rt}),
     * 2순위는 해외 현재가가 실어 주는 원화 환산 기준환율({@code currency_prc})이다. 둘 다
     * 장중에 초 단위로 움직이는 값이 아니라 그날의 기준환율이라, 20초로 잡아 봐야 같은 숫자를
     * 다시 받아 오는 데 상류 호출만 쓴다. 시세는 종목 가격이라 사정이 반대다.
     *
     * <p><b>왜 하필 10분인가</b> — 상한은 "틀린 값이 얼마나 오래 화면에 남아도 되나" 가 정한다.
     * 기준환율이 날짜가 바뀌거나 장중에 갱신되면 캐시가 만료될 때까지 옛 값이 나가는데,
     * 10분이면 원화 환산 오차가 보이기 전에 따라잡는다. 하한은 "한 번 물어보는 데 몇 콜이
     * 나가나" 가 정한다 — 환율 1회 조회는 계좌목록 + 잔고(+ 폴백 시 현재가)로
     * <b>최대 3콜</b>이고 그게 100ms 안에 몰려 나간다. 10분이면 사용자당 시간당 최대 18콜이다.
     *
     * <p>0 이하로 두면 캐시를 끈다(로컬 디버깅용).
     */
    private int fxCacheTtlSeconds = 600;

    /**
     * 환율을 <b>못 구했을 때</b>의 캐시 유지 시간 (초) — negative caching.
     *
     * <p>실패를 캐시하지 않으면 캐시가 아무 일도 안 한다. 환율이 안 나오는 사용자(해외 계좌도
     * 없고 폴백 종목도 막힌 경우)는 <b>매 요청마다</b> 3콜을 다시 내는데, 그게 정확히 지금
     * 429 를 부르는 모양이다. 그래서 "못 구했다" 는 사실 자체를 짧게 기억한다.
     *
     * <p>성공 캐시보다 훨씬 짧은 이유 — 실패는 대개 일시적이라(업스트림 5xx·타임아웃) 오래
     * 붙들면 복구가 늦어진다. 1분이면 상류 부담은 60배 줄면서 복구는 사람이 못 느낀다.
     */
    private int fxFailureCacheTtlSeconds = 60;

    /**
     * 429(유량 초과)를 맞은 뒤 환율을 다시 물어보기까지의 백오프 (초).
     *
     * <p><b>일반 실패와 나눠 두는 이유</b> — 429 는 "이번 호출이 실패했다" 가 아니라
     * "<b>지금 너무 많이 부르고 있다</b>" 는 신호다. 같은 1분 뒤에 다시 치면 원인을 그대로
     * 재현한다. 그래서 일반 실패({@link #fxFailureCacheTtlSeconds})보다 길게 쉰다.
     *
     * <p>5분인 근거는 dev 실측이다 — 429 짝이 <b>2~3분 간격</b>으로 반복됐으므로(2026-08-28)
     * 그 주기보다 길어야 반복이 끊긴다. 반대로 성공 캐시(10분)보다는 짧게 둬서 상류가 풀리면
     * 곧 따라잡게 한다.
     */
    private int fxRateLimitBackoffSeconds = 300;
