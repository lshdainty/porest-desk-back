...
class ExpenseBudgetServiceComplianceTest {
    ...
    /**
     * 카드 이월(D4)은 예산에 안 들어간다 — 카드를 만들 때 적은 "이전 미결제 사용액" 은
     * 앱을 쓰기 전에 이미 쓴 돈이라 그 달의 지출이 아니다. 종전엔 이 화면만 카드 쪽 규칙
     * ({@code countable}) 을 써서, 카드를 등록한 달의 이행률이 수백 % 로 솟았다.
     */
    @Test
    @DisplayName("카드 이월 30만은 예산 이행률에 안 들어간다")
    void cardCarryoverExcluded() {
        givenBudgets(List.of(overallBudget(100_000L)));
        Expense carryover = expense(ExpenseType.EXPENSE, 300_000L);
        carryover.markAutoGenerated(Expense.AUTO_SOURCE_CARD_CARRYOVER);
        givenExpenses(List.of(expense(ExpenseType.EXPENSE, 40_000L), carryover));

        var result = sut.getCompliance(USER_ID, 1);

        assertThat(result.get(0).totalSpent()).isEqualTo(40_000L);
        assertThat(result.get(0).compliancePercent()).isEqualTo(40.0);
    }
}
